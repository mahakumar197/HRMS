...
class attendancenot extends Command
{
     ...
     * @var string
     */
    protected $signature = 'attendancenot:logged';
    ...
    public function handle()
    {
        try {
            ...
            $emp_count = count($attend_not_marked);
            ...
            if ($today->isWeekday() == true && in_array($today->format('Y-m-d'), $hol_day) == false) {
                foreach ($super_admin as $sa) {
                    Mail::to($sa->email)->send(new attendsuperadmin([$attend_not_marked, $date, $emp_count]));
                }
